...
- (BOOL) readFromFile:(NSString *)path toColorListNamed: (NSString *) colorListName {
    ...
    for (NSString *line in lines) {
        ...
        NSRegularExpression *expr = [NSRegularExpression regularExpressionWithPattern:@"([^=]+)\\s*=\\s*#([a-f0-9]{3,8})" options:NSRegularExpressionCaseInsensitive error:nil];
        [expr enumerateMatchesInString:line options:0 range:NSMakeRange(0, line.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
            if (result.numberOfRanges == 3) {
                NSString *hex = [line substringWithRange:[result rangeAtIndex:2]];
                NSString *name = [line substringWithRange:[result rangeAtIndex:1]];
                ...
                @try {
                    ...
                    if (color) {
                        ...
                        [self.colors setColor:calibratedColor forKey:name];
                    }
